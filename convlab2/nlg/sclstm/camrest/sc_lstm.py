...
class SCLSTM(NLG):
    ...
    def generate_delex(self, dialog_acts):
        """
        dialog_acts = [[intent, slot, value],...]] => meta = {"inform": [["area","centre of town"]]}
        """
        # add placeholder value
        action = {}
        for intent, slot, value in dialog_acts:
            action.setdefault(intent, [])
            action[intent].append([slot, value])
        meta = action
