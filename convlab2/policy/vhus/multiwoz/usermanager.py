...
class UserDataManager(object):
    ...
    def da_list_form_to_dict_form(da):
        ...
        for [intent, domain, slot, value] in da:
            ...
            slot = ref_slot_data2stand.get(domain.lower(), {}).get(slot, slot)
            if domain_intent in ret:
                ret[domain_intent].append([slot, value])
            else:
                ret[domain_intent] = [[slot, value]]
        return eval(str(ret).lower())
    ...
    def da_dict_form_to_list_form(da):
        ...
        for key in da:
            ...
            for [slot, value] in da[key]:
                if slot != 'none':
                    slot = slot.capitalize()
                ret.append([intent, domain, slot, value])
        return ret


    def org_data_loader(self):
        if self.__org_goals is None or self.__org_usr_dass is None or self.__org_sys_dass is None:
            zip_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))),
