...
def read_langs3(source_text, utterance, gating_dict, SLOTS, dataset, lang, mem_lang, sequicity, training, max_line = None):
    data = []
    max_resp_len, max_value_len = 0, 0
    domain_counter = {}
    if 1 == 1:
...
def prepare_data_seq(training, task="dst", sequicity=0, batch_size=100):
    ...
    file_train = model_root + '/data/train_dials.json'
    file_dev = model_root + '/data/dev_dials.json'
    file_test = model_root + '/data/test_dials.json'
    ...
    ontology = json.load(open(os.path.join(model_root, "data/multi-woz/MULTIWOZ2 2/ontology.json"), 'r'))
...
def prepare_data_seq2(training, task="dst", sequicity=0, batch_size=100, source_text='', curr_utterance=''):
    training = False
    eval_batch = args["eval_batch"] if args["eval_batch"] else batch_size
    root_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

    file_train = root_path + '/data/train_dials.json'
    file_dev = root_path + '/data/dev_dials.json'
    ...
    # load domain-slot pairs from ontology
    ontology = json.load(open(root_path + "/data/multi-woz/MULTIWOZ2 2/ontology.json", 'r'))
    ALL_SLOTS = get_slot_information(ontology)
    gating_dict = {"ptr":0, "dontcare":1, "none":2}
    # Vocabulary
    lang, mem_lang = Lang(), Lang()
    ...
    if not training:
        ...
        # pair_test, test_max_len, slot_test = read_langs(file_test, gating_dict, ALL_SLOTS, "test", lang, mem_lang, sequicity, training)
        pair_test, test_max_len, slot_test = read_langs2(source_text, utterance, gating_dict, ALL_SLOTS, "test", lang,
                                                         mem_lang, sequicity, training)
        test  = get_seq(pair_test, lang, mem_lang, eval_batch, False, sequicity)
...
def prepare_data_seq_cn(training, task="dst", sequicity=0, batch_size=100):
    ...
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
    # load domain-slot pairs from ontology
    ontology = json.load(open("data/multi-woz/ontology.json", 'r'))
    ALL_SLOTS = get_slot_information(ontology)
    gating_dict = {"ptr":0, "dontcare":1, "none":2}
    # Vocabulary
    lang, mem_lang = Lang(), Lang()
