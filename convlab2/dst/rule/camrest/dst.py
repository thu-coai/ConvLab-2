...
class RuleDST(DST):
    ...
    Attributes:
        state(dict):
            Dialog state. Function ``convlab2.util.camrest.state.default_state`` returns a default state.
    """
    def __init__(self):
        super().__init__()
    ...
    def init_session(self):
        """Initialize ``self.state`` with a default state, which ``convlab2.util.camrest.state.default_state`` returns."""
        self.state = default_state()
...
if __name__ == '__main__':
    ...
    # Please call `init_session` before a new dialog. This initializes the attribute `state` of tracker with a default state, which `convlab2.util.camrest.state.default_state` returns. But You needn't call it before the first dialog, because tracker gets a default state in its constructor.
    dst.init_session()
